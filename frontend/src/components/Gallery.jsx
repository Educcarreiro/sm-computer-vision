    ...
    <div style={{
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <TeamBadge name={report.team_a?.name || 'Team A'} color="var(--accent-cyan)" />
          <span style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 700 }}>VS</span>
          <TeamBadge name={report.team_b?.name || 'Team B'} color="var(--accent-red)" />
          {item.created_at && (
            <span style={{
        ...
        <div style={{ padding: '0 16px 16px' }}>
          ...
          {report.matchup && (
            <div style={{
              ...
              <StyleBadge style={report.matchup.style} pressing={report.matchup.pressing_avg} />
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginTop: 8 }}>
                <MiniStat label="Gap" value={`${report.matchup.gap}m`} />
                <MiniStat label="Pressing" value={`${report.matchup.pressing_avg}m`} />
                <MiniStat label="Compacto" value={report.matchup.more_compact} />
                <MiniStat label="Largo" value={report.matchup.wider_play} />
                <MiniStat label="Agressivo" value={report.matchup.more_aggressive} />
              </div>
            </div>
          )}
        </div>
...
function StyleBadge({ style, pressing }) {
  const color = pressing < 8 ? 'var(--accent-red)'
    : pressing < 14 ? 'var(--accent-yellow)'
    : 'var(--accent-green)'
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        <MiniStat icon={<Target size={12} />} label="Formacao" value={topForm?.formation || '?'} sub={`${topForm?.percent || 0}%`} />
        <MiniStat icon={<Maximize2 size={12} />} label="Compactacao" value={`${team.compactness}m2`} />
